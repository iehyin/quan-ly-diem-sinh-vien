...
public class ConsoleUI {
    ...
    public ConsoleUI(DataStorage storage) {
    ...
    private void hienThiDanhSachSinhVien() {
        System.out.println("\n--- DANH SÁCH SINH VIÊN ---");
        if (storage.getDsSinhVien().isEmpty()) {
            System.out.println("Danh sách trống.");
            return;
        }
        for (SinhVien sv : storage.getDsSinhVien().values()) {
    ...
    private void giaoDienNhapDiemTheoMSSV() {
        ...
        System.out.println("\n--- DANH SÁCH MÔN HỌC ---");
        for (MonHoc mh : storage.getDsMonHoc().values()) {
        ...
        Diem diemMoi = new Diem(sv, mh, thuongKy, diemGK, diemCK);
        storage.themDiem(diemMoi);
    ...
    private void giaoDienTraCuuDiemTheoMSSV() {
        ...
        for (Diem diem : storage.getDsDiem()) {
